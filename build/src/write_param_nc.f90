...
contains
  ...
  subroutine defNetCDF(fname,           &  ! input: output nc filename
    ...
    do iBeta=1,nBetaOut
      ...
      call defvar(trim(betaMetaTemp(iBeta)%pname),trim(betaMetaTemp(iBeta)%desc),trim(betaMetaTemp(iBeta)%unt),(/defDim2%dimName,defDim1%dimName/),nf90_float,err,cmessage)
      if(err/=0)then; message=trim(message)//trim(nf90_strerror(err)); return; endif
    end do
    ! end definitions
